...
import { 
  ...
  ShieldCheck, Trophy, Briefcase, GraduationCap 
} from 'lucide-react';

/**
 * CONFIGURACIÓN DE NAVEGACIÓN (Definida internamente para compilación en Canvas)
 * En tu entorno local, puedes mantener esto en src/constants/navigation.ts
 */
...
const NAV_ITEMS_BY_ROLE: Record<string, NavItem[]> = {
  ...
  SuperAdmin: [
    ...
    { id: 'torneos', label: 'Torneos', icon: Trophy },
    { id: 'admin', label: 'Config', icon: Briefcase }
  ],
  Profesor: [
    { id: 'inicio', label: 'Inicio', icon: LayoutDashboard },
    { id: 'pagos', label: 'Pagos', icon: DollarSign },
    { id: 'alumnos', label: 'Alumnos', icon: Users },
    { id: 'examenes', label: 'Examenes', icon: GraduationCap },
        { id: 'torneos', label: 'Torneos', icon: Trophy }
...
export const GlobalNavbar: React.FC<GlobalNavbarProps> = ({ activeTab, onTabChange, role }) => {
  // Obtenemos los ítems correspondientes al rol, si no existe el rol, devolvemos un array vacío
    ...
    <div className="fixed bottom-0 left-0 right-0 p-6 pointer-events-none z-50">
      ...
      <nav className="max-w-sm mx-auto h-18 bg-[var(--color-card)]/80 backdrop-blur-2xl rounded-[2.2rem] border border-[var(--color-border)] shadow-[0_20px_50px_-10px_rgba(0,0,0,0.4)] flex justify-around items-center px-4 pointer-events-auto transition-all">
        {items.map((item) => (
          ...
          >
            {/* Indicador de pestaña activa con animación de resorte */}
            {activeTab === item.id && (
              <motion.div 
            ...
            {/* Icono dinámico de Lucide-React */}
            <item.icon 
              size={20} 
